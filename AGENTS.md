...
Before selecting or executing any Skyforge work, read `docs/agent-state/EXECUTION_BOUNDARIES.md`.
Its execution-location rules are canonical and override older issue text, lane notes, worker prompts,
historical PRs, and conflicting wording in the orchestration protocol.

The boundary is deliberately simple:

- **DigitalOcean `skyforge-orchestrator` droplet = orchestration control plane only.** It may run the
  orchestrator, controller bookkeeping, GitHub coordination, model-control/classification, and bounded
  repository-authoring handoffs. It must not be used as a project build/test/runtime machine.
- **GitHub / GitHub Actions = repository truth and all automated project machine work.** Compilation,
  Gradle/NeoForge execution, automated tests, benchmarks, deterministic evidence generation, and
  machine acceptance run there.
- **Project owner's local machine = manual/human-gated work.** Minecraft play/flight/visual review,
  listening/taste review, interactive inspection, and other qualitative/manual checks run locally.

Never move a workload to another surface merely because the intended surface is blocked. Fail closed,
repair the intended path, and preserve any in-progress repository edits without inventing acceptance.

## Canonical sources

- **Execution surfaces:** `docs/agent-state/EXECUTION_BOUNDARIES.md`
...
If summaries disagree with `main`, source/tests, or merged history, the repository evidence wins. For
**where work is allowed to execute**, `EXECUTION_BOUNDARIES.md` wins over older repository wording.
...
0. read `EXECUTION_BOUNDARIES.md`, then inspect hosted-controller ownership, controller-managed PRs,
   active external-producer claims, and healthy manual producer PRs before selecting work; for an
   interactive/manual producer, follow `MANUAL_PRODUCER_PROTOCOL.md` and obtain/verify the issue claim
   before editing;
...
9. continue the smallest technically prudent active milestone on the correct execution surface.
...
- Hosted workers may author bounded repository changes but must hand executable verification to GitHub Actions.
- Do not run project builds/tests/runtime evidence on the DigitalOcean orchestrator host, even when an older task says to do so.
- Do not perform human visual/play/listening/manual gates anywhere except the project owner's local machine unless the owner explicitly changes the canonical execution-boundary policy.
...
- Cheap deterministic evidence: broad/exhaustive where practical, on GitHub Actions.
- Expensive runtime/client evidence: representative by risk-equivalence class, on GitHub Actions when automatable; qualitative judgment remains local.
...
Waiting on CI is not work: do not poll repeatedly. Resume when useful evidence exists. A GitHub Actions
failure is machine evidence for the next bounded repair; it is not permission to rerun the project
workload on the droplet.
...
- required automated executable evidence comes from the required GitHub Actions run for the relevant head;
- required human gates were executed locally and recorded durably;
