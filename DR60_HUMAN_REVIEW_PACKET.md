...
- Packet base-main SHA before DR-60 packaging: `44d25cf0f49d3803d5a99df687d0672bf8461b59`.
- Exact DR-60 packet implementation commit: `ab5785d6845ebca9e0fca5097c00ce06f6a0f137`.
...
From the repository root in PowerShell, prepare a clean accepted B world first. For **exact packet reproduction**, pin the DR-60 implementation commit rather than assuming moving `main` still names this snapshot:

```powershell
git fetch origin
git switch --detach ab5785d6845ebca9e0fca5097c00ce06f6a0f137
git rev-parse HEAD
.\gradlew.bat :skyforge-neoforge-1211:dr50IntegratedRegionAcceptance --stacktrace --no-configuration-cache
```

For exact reproduction, `git rev-parse HEAD` must be `ab5785d6845ebca9e0fca5097c00ce06f6a0f137`. This commit contains both the persistent DR-60 human-review launcher and the packet itself. To review a newer `main` instead, switch back to `main`, pull it, and treat that as a new-current-main review after checking intervening changes; do not silently reuse an older prepared world.
