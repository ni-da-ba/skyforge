...
/**
 * Development-only stationary live proof for issue #237's optional Simulated Portable Engine
 * redstone cutoff.
 *
 * <p>The class references Simulated only through registry IDs/reflection. Production Skyforge
 * therefore remains loadable without the optional flight stack.
 */
final class SkyforgePortableEngineCutoffAcceptance {
    ...
    private static void onServerStarted(ServerStartedEvent event) {
        ...
        try {
            ...
            assertAbsFloat(
                    "default mode generates normal power",
                    32.0f,
                    getGeneratedSpeed.invoke(blockEntity));

            // 2. Opted-in cutoff freezes an active timer and produces zero output.
            ...
            assertInt(
                    "cutoff preserves active burn timer",
                    100,
                    getBurnTime.invoke(blockEntity));
            assertFloat(
                    "cutoff produces zero generator output",
                    0.0f,
                    getGeneratedSpeed.invoke(blockEntity));
            ...
            assertInt(
                    "cutoff preserves zero pre-ignition timer",
                    0,
                    getBurnTime.invoke(blockEntity));
            ...
            // 4. Removing signal starts the same engine normally from queued fuel.
            ...
            assertInt(
                    "restart consumes exactly one queued fuel item",
                    1,
                    afterStart.getCount());

            // Simulated computes isLit before loading newly queued fuel, so cold ignition has one
            // upstream unpowered tick. Preserve and characterize that behavior.
            assertFloat(
                    "cold ignition first tick remains unpowered",
                    0.0f,
                    getGeneratedSpeed.invoke(blockEntity));
            tick.invoke(blockEntity);
            int runningBurn = ignitionBurn - 1;
            assertInt(
                    "second ignition tick begins countdown",
                    runningBurn,
                    getBurnTime.invoke(blockEntity));
            assertAbsFloat(
                    "second ignition tick restores normal generator output",
                    32.0f,
                    getGeneratedSpeed.invoke(blockEntity));
            ...
            assertInt(
                    "second cutoff preserves exact timer",
                    runningBurn,
                    getBurnTime.invoke(blockEntity));
            assertFloat(
                    "second cutoff removes output",
                    0.0f,
                    getGeneratedSpeed.invoke(blockEntity));
            ...
            tick.invoke(blockEntity);
            assertInt(
                    "restart resumes countdown",
                    runningBurn - 1,
                    getBurnTime.invoke(blockEntity));
            assertAbsFloat(
                    "warm restart restores generator output",
                    32.0f,
                    getGeneratedSpeed.invoke(blockEntity));
            ...
            throw new IllegalStateException(
                    "Portable Engine cutoff acceptance reflection failure",
                    cause);
        } finally {
            level.setBlock(CUTOFF_POWER_POS, Blocks.AIR.defaultBlockState(), 3);
    ...
    private static void fail(String reason) {
        LOGGER.log(
                ...
                "PORTABLE_ENGINE_CUTOFF_ACCEPTANCE FAIL " + reason);
        throw new IllegalStateException(
                "Portable Engine cutoff acceptance failed: " + reason);
    }
}
