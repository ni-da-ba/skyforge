...
/**
 * Bounded AUTH-0093/C20 Iron realization for one admitted exact Skyforge volume.
 *
 * <p>This deliberately contains one implementation-owned, surface-exposed ore block. Its quantity,
 * grade, count, and access are explicit constants rather than interpretations of normalized
 * Authorship opportunity. The terrain block itself is the persisted authority: after a chunk save,
 * deterministic replanning identifies the same owned coordinate and does not add a second deposit.
 */
final class SkyforgeIronDepositAdapter {
    static final ResourceLocation IRON_ORE = ResourceLocation.withDefaultNamespace("iron_ore");
    ...
    record Specification(int blockCount, Grade grade, Accessibility accessibility) {
        Specification {
            if (blockCount != 1) {
                throw new IllegalArgumentException("the bounded Iron representative contains exactly one block");
            }
    ...
    private SkyforgeIronDepositAdapter() {}

    /**
     * Selects the first exact owned top block in stable X/Z order. Zero AUTH-0093 opportunity
     * fails closed; C20's starting-cluster guarantee remains a scope-selection responsibility.
     */
    static Optional<Deployment> plan(
        ...
        if (!SkyIslandBaseMetalContentPolicy.geologicallyEligible(opportunity, SkyIslandBaseMetalKind.IRON)) {
    ...
    /** Writes the retained vanilla Iron identity once; a saved/reloaded block is an idempotent replay. */
    static Placement apply(ChunkAccess chunk, Deployment deployment) {
        ...
        if (!chunk.getPos().equals(deployment.chunkPos())) {
            throw new IllegalArgumentException("Iron deployment belongs to a different chunk");
        }
        if (chunk.getBlockState(deployment.position()).is(Blocks.IRON_ORE)) {
            return new Placement(deployment, false);
        }
        if (chunk.getBlockState(deployment.position()).isAir()) {
            throw new IllegalStateException("Iron deployment lost its exact owned surface support");
        }
        chunk.setBlockState(deployment.position(), Blocks.IRON_ORE.defaultBlockState(), false);
