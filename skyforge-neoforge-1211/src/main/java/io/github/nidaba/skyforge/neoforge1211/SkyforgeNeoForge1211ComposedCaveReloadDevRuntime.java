...
/** Full-stop/reload verifier for persisted SF-IMP-0067 native/authored cave union. */
@EventBusSubscriber(modid = SkyforgeNeoForge1211Mod.MOD_ID)
final class SkyforgeNeoForge1211ComposedCaveReloadDevRuntime {
    static final String ENABLE_PROPERTY = "skyforge.dev.composedCaveReload";
    static final String EXPECTED_RESULT_PROPERTY = "skyforge.dev.composedCaveExpectedResultFile";

    private static final System.Logger LOGGER =
            System.getLogger(SkyforgeNeoForge1211ComposedCaveReloadDevRuntime.class.getName());
    private static volatile ClientExpectation clientExpectation;
    private static boolean proofStarted;
    private static boolean proofComplete;

    private SkyforgeNeoForge1211ComposedCaveReloadDevRuntime() {}
    ...
    private static synchronized void verify(ServerLevel level) {
        ...
        if (SkyforgeNeoForge1211SurfaceStage.hasActiveBinding()
            ...
            throw new IllegalStateException(
                    "SF-IMP-0067 reload must not reinstall terrain/admission/cave mutation");
        }
        ...
        if (!nativeOnlyState.isAir()
                ...
                || !baseState.isAir()) {
            throw new IllegalStateException(
                    "SF-IMP-0067 reload lost composed cave union: nativeOnly=" + nativeOnly
                            + "=" + nativeOnlyState + ", mouth=" + mouth + "=" + mouthState
        ...
        LOGGER.log(
                ...
                "SF-IMP-0067 RELOAD SERVER PASS: nativeOnly=" + nativeOnly
                        + ", mouth=" + mouth + ", outward=" + outward + ", base=" + base
                        + ". Both native-only and authored cave AIR survived full stop/reload "
    ...
    private static Properties loadExpected() {
        ...
        if (configured == null || configured.isBlank()) {
            throw new IllegalStateException(
                    "SF-IMP-0067 reload requires " + EXPECTED_RESULT_PROPERTY);
        }
        Path path = Path.of(configured).toAbsolutePath().normalize();
        if (!Files.isRegularFile(path)) {
            throw new IllegalStateException("SF-IMP-0067 expected result is missing: " + path);
        }
        Properties properties = new Properties();
        try (InputStream input = Files.newInputStream(path)) {
            properties.load(input);
        } catch (IOException exception) {
            throw new IllegalStateException("failed to read SF-IMP-0067 result " + path, exception);
        }
        return properties;
    }
    ...
    private static String required(Properties properties, String key) {
        return Objects.requireNonNull(
                ...
                () -> "SF-IMP-0067 expected result missing " + key);
    }
