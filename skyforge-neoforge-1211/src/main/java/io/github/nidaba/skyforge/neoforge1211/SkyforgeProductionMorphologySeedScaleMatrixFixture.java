...
/**
 * Exact SF-IMP-0083 family-atlas fixtures for the remaining built-in AUTH-0083 seed/scale matrix.
 *
 * <p>Each family fixture carries four exact authored specimens: MEDIUM seed-min, MEDIUM seed-zero,
 * MEDIUM seed-skyforge, and LARGE seed-skyforge. The specimens retain identical X/Z coordinates
 * and are separated only by integer Y translations inside a development-only tall review
 * dimension. This deliberately reuses the accepted vertically stacked exact-volume lifecycle while
 * reducing the 20-member persistence/review matrix to five family worlds.
 */
final class SkyforgeProductionMorphologySeedScaleMatrixFixture {
    ...
    private static final int FIRST_TARGET_MINIMUM_SOLID_Y = 336;
    private static final int INTER_MEMBER_AIR_GAP = 32;
    ...
    static FamilyFixture buildFamily(MorphologyFamily family) {
        Objects.requireNonNull(family, "family");
        var compiler = new SkyIslandMorphologySpecCompiler();
        var registry = SkyIslandMorphologyProviders.builtInRegistry();
        var profile = SkyIslandTerrainProfile.reference();

        List<MemberFixture> memberFixtures = new ArrayList<>();
        List<SkyIslandWorldVolume> worldVolumes = new ArrayList<>();
        LinkedHashSet<Long> footprint = new LinkedHashSet<>();
        int targetMinimumSolidY = FIRST_TARGET_MINIMUM_SOLID_Y;
        int previousMaximumSolidY = Integer.MIN_VALUE;

        List<Member> familyMembers = members(family);
        if (familyMembers.size() != 4) {
            throw new IllegalStateException(
                    "SF-IMP-0083 family must contain exactly four remaining members: " + family);
        }

        for (int index = 0; index < familyMembers.size(); index++) {
            Member member = familyMembers.get(index);
            ...
            int verticalTranslation = Math.subtractExact(targetMinimumSolidY, sourceMinimumY);
            ...
            if (translatedMinimumY != targetMinimumSolidY) {
                throw new IllegalStateException(
                        member.id() + " did not land on its exact review-stack minimum Y");
            }
            if (translatedMinimumY <= 320) {
                throw new IllegalStateException(
                        member.id() + " must remain wholly above vanilla Overworld generation");
            }
            ...
            if (previousMaximumSolidY != Integer.MIN_VALUE
                    && translatedMinimumY - previousMaximumSolidY - 1 < INTER_MEMBER_AIR_GAP) {
                throw new IllegalStateException(
                        member.id() + " violated the exact stacked-review air gap");
            }

            SkyIslandWorldVolumeId volumeId = new SkyIslandWorldVolumeId(
                    ROOT_SEED,
                    "sf-imp-0083-morphology-seed-scale-" + family.identifier(),
                    family.ordinal(),
                    index,
                    member.seed());
            ...
            previousMaximumSolidY = translatedMaximumY;
            targetMinimumSolidY = Math.addExact(
                    Math.addExact(translatedMaximumY, 1),
                    INTER_MEMBER_AIR_GAP);
        }

        return new FamilyFixture(
                family,
                memberFixtures,
                new SkyIslandWorldCatalog(ROOT_SEED, worldVolumes),
                footprint);
    }
    ...
    record FamilyFixture(
        ...
        FamilyFixture {
            ...
            footprintChunkKeys = Set.copyOf(Objects.requireNonNull(footprintChunkKeys, "footprintChunkKeys"));
            if (members.size() != 4 || catalog.volumes().size() != 4 || footprintChunkKeys.isEmpty()) {
                throw new IllegalArgumentException("SF-IMP-0083 family fixture must contain four exact volumes");
            }
        }

        MemberFixture member(String commandId) {
            return members.stream()
                    .filter(member -> member.member().commandId().equals(commandId))
                    .findFirst()
                    .orElseThrow(() -> new IllegalArgumentException(
                            "unknown SF-IMP-0083 family member command: " + commandId));
        }
    }
}
