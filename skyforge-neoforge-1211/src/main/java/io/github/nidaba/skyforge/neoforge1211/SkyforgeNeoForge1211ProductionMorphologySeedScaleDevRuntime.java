...
/**
 * SF-IMP-0083 exact Minecraft carrier for one selected AUTH-0083 built-in seed/scale specimen.
 *
 * <p>Cheap deterministic tests remain exhaustive over all twenty members. This runtime intentionally
 * realizes one exact volume at a time so expensive lifecycle evidence cannot recreate the retired
 * four-member tall-atlas memory pathology. Cave/ecology/interior mutation remains absent.
 */
...
final class SkyforgeNeoForge1211ProductionMorphologySeedScaleDevRuntime {
    ...
    static final String MEMBER_PROPERTY = "skyforge.dev.productionMorphologySeedScaleMember";
    ...
    static String selectedMemberCommand() {
        ...
        if (value == null || value.isBlank()) {
            throw new IllegalStateException("SF-IMP-0083 seed/scale member property is not configured");
        }
        return value;
    }
    ...
    static synchronized void installFromSystemProperty() {
        ...
        LOGGER.log(
                ...
                "SF-IMP-0083 seed/scale single-member carrier enabled: family="
    ...
    private static synchronized void evaluate(ServerLevel level, ServerTickEvent.Post event) {
        ...
        if (level.getMinBuildHeight()
            ...
            SkyforgeAutomatedAcceptanceHarness.fail(
                    event.getServer(),
                    "SF-IMP-0083 single-member carrier loaded in the wrong dimension interval: minY="
    ...
    record SurfaceEvidence(
        ...
        boolean valid() {
            return claimedColumns > 0
                    ...
                    && heightMismatches == 0
                    && landTopBlocks > 0
                    // Grass remains useful presentation evidence, but this development-only
                    // high-altitude carrier deliberately has no native Overworld surface beneath
                    // it to copy. Morphology correctness requires stable land substrate, exact
                    // occupancy boundaries, exterior air, and persistence—not a specific top block.
