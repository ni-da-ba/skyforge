 ...
 *
 * <p>It deliberately selects no watershed or water availability: for each authored kind that is
 * present, the deployment uses the first stable accepted intent of that kind. An absent accepted
 * intent creates no deployment and never causes a substitute water feature to be invented. Water
 * replaces only exact owner voxels, which keeps this initial static realization out of the native-
 * spring provenance/propagation path.
 */
final class SkyforgeAuthoredVisibleHydrologyAdapter {
    ...
    static List<Deployment> plan(
        ...
        intent.channels().stream().findFirst().ifPresent(channel ->
                deployments.add(atPath(volume, terrain, Feature.CHANNEL, channel.path().points())));
        intent.retainedWater().stream().findFirst().ifPresent(retained ->
