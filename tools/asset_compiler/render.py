...
ROLE_FILL = {
    ...
    "lighting": "#d7b55b", "signal_mast": "#4e5965",
}
...
def write_isometric_svg(path: Path, compiled: CompiledAsset, scale: int = 9) -> None:
    max_x, max_y, max_z = compiled.summary["layout"]["bounds"]["max"]
    ox, oy = (max_z + 2) * scale + 40, (max_y + 3) * scale + 20
    width = (max_x + max_z + 6) * scale + 100
    height = (max_y * 2 + max_x + max_z + 8) * scale // 2 + 100
    project = lambda x, y, z: (ox + (x-z)*scale, oy + (x+z)*scale*0.5 - y*scale)
    ...
    occupied = set(compiled.model.cells)
    for (x, y, z), cell in sorted(compiled.model.cells.items(), key=lambda item: (sum(item[0]), item[0][1], item[0][0])):
        base = ROLE_FILL.get(cell.role, "#aaa")
        faces = []
        if (x, y+1, z) not in occupied:
            faces.append(([project(x,y+1,z), project(x+1,y+1,z), project(x+1,y+1,z+1), project(x,y+1,z+1)], shade(base,1.10)))
        if (x+1, y, z) not in occupied:
            faces.append(([project(x+1,y,z), project(x+1,y+1,z), project(x+1,y+1,z+1), project(x+1,y,z+1)], shade(base,0.83)))
        if (x, y, z+1) not in occupied:
            faces.append(([project(x,y,z+1), project(x+1,y,z+1), project(x+1,y+1,z+1), project(x,y+1,z+1)], shade(base,0.95)))
