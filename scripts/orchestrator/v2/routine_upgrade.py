...
class RoutineUpgradeController:
    ...
    def _legacy_projection_blockers(
        ...
        *,
        ...
        try:
            ...
            elif events and len(snapshot_covered) == len(events):
    ...
    def _retire_snapshot_covered_ordinary_noise(self, target_sha: str) -> int:
        """Retire only ordinary fallback events already represented by target_sha."""
    ...
    def _quiesce_snapshot_covered_ordinary_noise(self, target_sha: str) -> int:
        ...
        if not events or len(self._snapshot_covered_ordinary_events(raw, target_sha)) != len(events):
            return 0
    ...
    def _wait_for_legacy_quiescence(
        ...
        *,
        ...
        try:
            ...
            return [f"legacy bounded lifecycle quiescence failed: {type(exc).__name__}: {exc}"]
        if retired:
