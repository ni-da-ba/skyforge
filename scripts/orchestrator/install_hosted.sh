...
if [[ -z "${SKYFORGE_DEVELOPMENT_API_TOKEN:-}" && -f "$ENV_FILE" ]]; then
  SKYFORGE_DEVELOPMENT_API_TOKEN="$(sudo sed -n 's/^SKYFORGE_DEVELOPMENT_API_TOKEN=//p' "$ENV_FILE" | head -n 1)"
fi
if [[ -z "${SKYFORGE_DEVELOPMENT_WRITE_TOKEN:-}" && -f "$ENV_FILE" ]]; then
  SKYFORGE_DEVELOPMENT_WRITE_TOKEN="$(sudo sed -n 's/^SKYFORGE_DEVELOPMENT_WRITE_TOKEN=//p' "$ENV_FILE" | head -n 1)"
fi
if [[ -z "${SKYFORGE_DEVELOPMENT_WRITE_ACTOR:-}" && -f "$ENV_FILE" ]]; then
  SKYFORGE_DEVELOPMENT_WRITE_ACTOR="$(sudo sed -n 's/^SKYFORGE_DEVELOPMENT_WRITE_ACTOR=//p' "$ENV_FILE" | head -n 1)"
fi
...
if [[ -n "${SKYFORGE_DEVELOPMENT_API_TOKEN:-}" && "${#SKYFORGE_DEVELOPMENT_API_TOKEN}" -lt 32 ]]; then
  echo "SKYFORGE_DEVELOPMENT_API_TOKEN must be at least 32 characters when configured." >&2
  exit 1
fi
if [[ -n "${SKYFORGE_DEVELOPMENT_WRITE_TOKEN:-}" && "${#SKYFORGE_DEVELOPMENT_WRITE_TOKEN}" -lt 32 ]]; then
  echo "SKYFORGE_DEVELOPMENT_WRITE_TOKEN must be at least 32 characters when configured." >&2
  exit 1
fi
if [[ -n "${SKYFORGE_DEVELOPMENT_WRITE_TOKEN:-}" && -z "${SKYFORGE_DEVELOPMENT_WRITE_ACTOR:-}" ]] || \
   [[ -z "${SKYFORGE_DEVELOPMENT_WRITE_TOKEN:-}" && -n "${SKYFORGE_DEVELOPMENT_WRITE_ACTOR:-}" ]]; then
  echo "SKYFORGE_DEVELOPMENT_WRITE_TOKEN and SKYFORGE_DEVELOPMENT_WRITE_ACTOR must be configured together." >&2
  exit 1
fi
