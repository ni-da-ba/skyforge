...
class HostedExecutionCoordinatorTest(unittest.TestCase):
    ...
    def test_new_signed_revision_supersedes_preclassifier_plan_without_classifier_call(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            old_request_id = plan.seed.classifier_request.request_id
            self.assertIsNone(
                ClassifierRunStore.for_root(root).load().get(old_request_id)
            )
            ...
            revised["comment"]["body"] = revised["comment"]["body"].replace(
                ...
                "Implement bounded feature current revision",
            )
    ...
    def test_new_signed_revision_supersedes_nonexecuted_blocked_admission_before_retry(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            base = make_repo(root)
            write_legacy(root)
            gate = ready_gate(base)
            app = self.restart(root, gate)
            raw, headers = signed(task_payload(), delivery="r5c24-blocked-original")
            status, response = app.handle_webhook(headers=headers, raw=raw)
            self.assertEqual(status, 202)
            self.assertTrue(response["task_authority_recorded"])
            ...
            revised["comment"]["created_at"] = "2026-09-18T03:04:00Z"
            revised["comment"]["updated_at"] = "2026-09-18T03:04:00Z"
            revised["comment"]["body"] = revised["comment"]["body"].replace(
                "Implement bounded feature",
                "Implement bounded feature current blocked revision",
            )
            raw2, headers2 = signed(revised, delivery="r5c24-blocked-revision")
            status2, response2 = app.handle_webhook(headers=headers2, raw=raw2)
            self.assertEqual(status2, 202)
            self.assertTrue(response2["task_authority_recorded"])
