...
def _control_plane_roadmap_resolve_active(
    ...
    if isinstance(active, dict) and active.get("issue_number"):
        ...
        with self._state_lock:
            ...
            dispatch_record = records.get(authority_key)
            dispatch_record = dict(dispatch_record) if isinstance(dispatch_record, dict) else None
            ...
            if not assigned:
                try:
                    _assign_github_agent(
                        self,
                        issue_number=issue_number,
                        lane=str(dispatch_record.get("lane") or active.get("lane") or "Implementation"),
                        objective=str(dispatch_record.get("objective") or "Resume the bounded issue objective."),
                        stop_boundary=str(dispatch_record.get("stop_boundary") or "Honor the issue stop boundary."),
                    )
                except Exception as exc:
                    state["last_error"] = {
                        "at": core._utc_now(),
