...
These values remain analytic hypotheses from historical PR #242. They are **not** acceptance
thresholds except for the existing `>60 kpg` explicit-review rule.

C12 B0-A1/A2 now reads `ServerSubLevel.getMassTracker()` from the real assembled MAIN_BODY. Any
difference between live and paper mass/CG is preserved as evidence and drives the next aircraft
layout decision; it must not be compatibility-normalized back to this estimate.
