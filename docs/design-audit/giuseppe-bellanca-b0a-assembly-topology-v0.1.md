...
## 2026-09-08 current-main recomposition note

Historical PR #242 closed unmerged, so this document is **not** accepted aircraft behavior by itself.
C12 has recomposed its exact source-constrained topology onto current `main` only as input to the
live B0-A1/A2 specimen.

The first executable slice intentionally stops before PROP_CHILD, control children, landing gear,
and flight. It must prove the 105 unique MAIN_BODY positions through the real Physics Assembler and
replace the paper mass/CG estimate with Sable's own live mass tracker.
