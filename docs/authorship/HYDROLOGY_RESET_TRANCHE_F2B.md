# Hydrology reset tranche F2B — ordinary bounded-profile assembly
...
F2B is the first consumer of the F2 numerical primitive. It replaces the historical topological
...
with `semanticWeight_i > 0`. The initial F2B policy uses `semanticWeight_i = 1` unless later
...
For each ordinary sample, F2B derives an admissible interval `L_i <= H_i <= U_i` from the same
...
F2B only assembles transition-free ordinary components.
...
identity. F2B must not create a second routing authority.
...
The next transition tranche may proceed only after F2A is accepted and F2B proves that the bounded
