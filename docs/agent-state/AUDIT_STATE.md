...
| --- | --- | --- |
| Implementation — SF-IMP-0083 / PR #285 | **LOOP RISK — unchanged expensive matrix restarted** | The first profiling assertion was corrected with information gain and retained regressions are green. But all five seed/scale prepares then spent ~54–55 minutes in synchronous warmup versus ~3–4 minutes for SF-IMP-0082. A documentation-only current-main merge (`9b0deaee`) cancelled those runs through workflow concurrency and immediately restarted the same runtime/fixture/warmup matrix unchanged. Stop full-matrix reruns; collect targeted footprint/admission/warmup profiling or change packaging first. If materially unchanged reruns continue, hand SF-IMP-0083 to a fresh Implementation session. |
