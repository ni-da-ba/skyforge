...
No MUS milestone is merged on `main` yet.

`MUS-0001` / PR #159 is the first merge candidate. The branch has been synchronized onto current
`main`; exact-head CI and merge remain the acceptance boundary.

## Current accepted musical state on the candidate branch
...
1. land MUS-0001 cleanly after exact-head CI;
2. update this ledger and cross-lane contracts to MERGED / ACCEPTED on a fresh current-main branch;
3. close Track 00 repair through human A/B;
4. recover HC state for frozen cues;
5. recover and audit exact Track 06 Draft 02.3 MIDI;
6. only then resume principal-theme composition.
