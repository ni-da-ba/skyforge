...
6. Implement the signal-free island descriptor and recipe.
7. Implement the reference sampler, raster outputs, statistics, and evidence manifest.
8. Pass signal-free island gates.
...
| SF-OPEN-006 | Canonical graph/descriptor serialization rules | serialization work | round-trip prototype may remain non-canonical and cannot supply release checksums |

SF-OPEN-001 through SF-OPEN-004 are resolved by ADR-0007. A future public distribution requires a new explicit license decision.
...
**Ticket:** SF-IMP-0001 - Repository and kernel skeleton
**Objective:** Produce a clean multi-module build in which an empty backend-neutral kernel can be compiled and tested without Minecraft dependencies.
**Done when:**

- the chosen Java toolchain and Gradle wrapper are pinned;
- the four v0.1 modules compile;
- module dependency rules are enforced by automated tests;
- CI performs a clean build and test;
- a minimal `Field2<Double>`-equivalent contract and one constant-field test pass;
- no island recipe, noise library, or Minecraft adapter has been added prematurely.
