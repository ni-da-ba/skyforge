...
**Ticket:** SF-IMP-0007 - Signal-free island gate suite
**Objective:** Execute the fixed evidence corpus and formalize every currently applicable
signal-free island acceptance gate as a named, repeatable test.
**Done when:**

- the fixed descriptor corpus passes closed-landform, bounded-elevation, scale, elevation, ridge,
  zero-signal, density-consistency, and explainability requirements;
- every measured tolerance and envelope is recorded rather than implied by screenshots;
- the standard evidence command completes from a clean checkout and its golden checksums are pinned;
- gates that require seeded variation remain explicitly deferred until seed derivation is accepted.
