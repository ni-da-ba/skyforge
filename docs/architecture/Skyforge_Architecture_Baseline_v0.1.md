...
**Ticket:** SF-IMP-0006 - Reference evidence pipeline
**Objective:** Sample the first compiled island through backend-neutral fields and emit the fixed
numerical and visual evidence needed to evaluate the signal-free island gates.
**Done when:**

- a descriptor compiles and samples over configurable deterministic grids;
- height, land-mask, slope, and orthogonal cross-section data are generated;
- statistics and checksums are recorded in a versioned evidence manifest;
- standard raster artifacts can be regenerated by one documented command;
- no Minecraft adapter or seeded signal is introduced.
